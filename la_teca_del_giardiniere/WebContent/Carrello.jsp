...
<head>
    ...
    <style>
        ...
        .message.warning { /* Aggiungi stile per i messaggi di warning */
...
<body>
    ...
    <div class="cart-container">
        ...
        <%-- Messaggi di Feedback (Successo/Errore/Warning) --%>
        ...
        <%-- L'attributo si chiama "carrello" e contiene una Map --%>
        <c:choose>
            <c:when test="${empty requestScope.carrello}">
            ...
            <c:otherwise>
                <table class="cart-table">
                    ...
                    <tbody>
                        <%-- Iterate over the Map entries. 'entry' will have 'key' (idPianta string) and 'value' (RigaCarrello object) --%>
                        <c:forEach var="entry" items="${requestScope.carrello}">
                            <c:set var="riga" value="${entry.value}" />
                            <c:set var="pianta" value="${riga.pianta}" /> <%-- Assuming RigaCarrello has getPianta() --%>
                            <tr>
                                <td>
                                    <c:out value="${pianta.nomeComune}" />
                                    <br>
                                    <small>(ID: ${pianta.id})</small> <%-- Utile per debug, puoi rimuoverlo in produzione --%>
                                </td>
                                <td><fmt:formatNumber value="${pianta.prezzo}" type="currency" currencySymbol="€" minFractionDigits="2"/></td>
                                <td>
                                    <form action="${pageContext.request.contextPath}/aggiornaQuantitaCarrello" method="post" style="display:inline-flex; align-items:center;">
                                        <%-- Passiamo l'ID della pianta come chiave per l'HashMap --%>
                                        <input type="hidden" name="idPianta" value="${pianta.id}"> 
                                        <input type="number" name="quantita" value="${riga.quantita}" min="0" class="quantity-input" onchange="this.form.submit()">
                                        <%-- Rimuoviamo il pulsante "Aggiorna" separato se l'aggiornamento avviene onchange --%>
                                        <%-- <button type="submit" class="btn small-btn">Aggiorna</button> --%>
                                    </form>
                                </td>
                                <td>
                                    <%-- Calcoliamo il subtotale della riga qui nella JSP --%>
                                    <fmt:formatNumber value="${pianta.prezzo * riga.quantita}" type="currency" currencySymbol="€" minFractionDigits="2"/>
                                </td>
                                <td>
                                    <form action="${pageContext.request.contextPath}/aggiornaQuantitaCarrello" method="post">
                                        <%-- Per rimuovere, impostiamo la quantità a 0 o usiamo un parametro specifico per la rimozione --%>
                                        <input type="hidden" name="Id" value="${pianta.Id}">
                                        <input type="hidden" name="quantita" value="0"> <%-- Imposta quantità a 0 per rimuovere --%>
                                        <button type="submit" class="btn remove-btn" onclick="return confirm('Sei sicuro di voler rimuovere ${pianta.nomeComune} dal carrello?');">Rimuovi</button>
                                    </form>
                                </td>
                            </tr>
                        </c:forEach>
                ...
                <div class="cart-actions">
                    ...
                    <a href="${pageContext.request.contextPath}/processaCheckout" class="btn">Procedi al Checkout</a> <%-- Assicurati il percorso corretto per la servlet di checkout --%>
                </div>
            </c:otherwise>
        </c:choose>
    </div>
