...
public class ModificaAccessorioServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            int accessorio_id = Integer.parseInt(idParam);
            Accessori accessorioDaModificare = accessoriDAO.getAccessorioByaccessorio_id(accessorio_id);

            if (accessorioDaModificare == null) {
                LOGGER.warning("Accessorio con ID " + accessorio_id + " non trovato per la modifica.");
                request.getSession().setAttribute("messaggio", "Accessorio non trovato per ID: " + accessorio_id);
                request.getSession().setAttribute("tipoMessaggio", "error");
                response.sendRedirect(request.getContextPath() + "/admin/ListaAccessoriServlet");
                return;
            }
            ...
            request.setAttribute("accessorio", accessorioDaModificare);
            LOGGER.info("Preparazione form per modifica accessorio con ID: " + accessorio_id);
            request.getRequestDispatcher("/admin/FormInserimentoAccessori.jsp").forward(request, response);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Accessori accessorio = buildAccessorioFromRequest(request, errori);
        ...
        try {
            boolean updated = accessoriDAO.updateAccessori(accessorio);
            if (updated) {
                ...
                request.getSession().setAttribute("messaggio", "Nessun accessorio trovato per l'aggiornamento.");
                request.getSession().setAttribute("tipoMessaggio", "error");
                LOGGER.warning("Fallimento nell'aggiornamento dell'accessorio, ID non trovato: " + accessorio.getAccessorio_id());
            }
            response.sendRedirect(request.getContextPath() + "/admin/ListaAccessoriServlet");
        } catch (SQLException e) {
    ...
    private Accessori buildAccessorioFromRequest(HttpServletRequest request, List<String> errori) {
        ...
        accessorio.setDescrizioneBreve(request.getParameter("descrizioneBreve"));
        accessorio.setDescrizioneDettagliata(request.getParameter("descrizioneDettagliata"));
        accessorio.setDimensioni(request.getParameter("dimensioni"));
        accessorio.setImmagine(request.getParameter("immagine"));
        accessorio.setCategoria(request.getParameter("categoria"));
