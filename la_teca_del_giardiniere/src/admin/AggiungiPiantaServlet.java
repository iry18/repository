...
import util.ServletUtils; // Ensure this utility class exists and is correct
...
public class AggiungiPiantaServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        // Assume ServletUtils.checkAuthenticationAndAuthorization is correctly implemented
        ...
        // Correct path for forwarding the JSP
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.setCharacterEncoding("UTF-8"); // Important for correct character handling

        String contextPath = request.getContextPath();
        List<String> errori = new ArrayList<>();
        
        // This is the crucial part: buildPiantaFromRequest should handle validation
        ...
        if (!errori.isEmpty()) {
            // If there are validation errors, return to the form with messages
            request.setAttribute("erroriModifica", errori); // Renamed attribute for clarity, can be "errori"
            request.setAttribute("pianta", pianta); // Return the partially filled plant object
            request.setAttribute("modalita", "inserisci");
        ...
        if (pianta != null && pianta.getPrezzo() == null) {
            LOGGER.severe("DEBUG: La proprietà 'prezzo' dell'oggetto Pianta è NULL prima dell'invio al DAO!");
        } else if (pianta != null) {
            LOGGER.info("DEBUG: La proprietà 'prezzo' dell'oggetto Pianta è: " + pianta.getPrezzo());
        } //else {
           // LOGGER.severe("DEBUG: L'oggetto Pianta è NULL prima dell'invio al DAO!");}//

        try {
            pianteDAO.aggiungiPianta(pianta);
            // Set success message in session for Post-Redirect-Get pattern
            ...
            LOGGER.log(Level.SEVERE, "Errore SQL durante l'aggiunta della pianta: " + (pianta != null ? pianta.getNomeComune() : "null (pianta object was null)"), e);
            List<String> sqlErrors = new ArrayList<>();
            // Make the error message more user-friendly but still informative
            if (e.getMessage() != null && e.getMessage().contains("Column 'NomeComune' cannot be null")) {
                 sqlErrors.add("Il campo 'Nome Comune' non può essere vuoto. Per favore, inserisci un valore.");
            } else {
